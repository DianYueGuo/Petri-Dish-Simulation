...
void GameSimulationController::update_creatures(const b2WorldId&, float dt) {
    for (size_t i = 0; i < ctx.sim_circles().size(); ++i) {
        if (ctx.sim_circles()[i] && ctx.sim_circles()[i]->get_kind() == CircleKind::Creature) {
            ...
            behavior_ctx.boost_area = ctx.cc_boost_area();
            behavior_ctx.circle_density = ctx.cc_circle_density();
            behavior_ctx.boost_particle_impulse_fraction = ctx.cc_boost_particle_impulse_fraction();
            behavior_ctx.boost_particle_linear_damping = ctx.cc_boost_particle_linear_damping();
            behavior_ctx.linear_impulse_magnitude = ctx.cc_linear_impulse_magnitude();
            behavior_ctx.angular_impulse_magnitude = ctx.cc_angular_impulse_magnitude();
            behavior_ctx.angular_damping = ctx.cc_angular_damping();
            behavior_ctx.live_mutation_enabled = ctx.cc_live_mutation_enabled();
            behavior_ctx.mutate_weight_thresh = ctx.cc_mutate_weight_thresh();
            behavior_ctx.mutate_weight_full_change_thresh = ctx.cc_mutate_weight_full_change_thresh();
            behavior_ctx.mutate_weight_factor = ctx.cc_mutate_weight_factor();
            behavior_ctx.tick_add_connection_thresh = ctx.cc_tick_add_connection_thresh();
            behavior_ctx.tick_add_node_thresh = ctx.cc_tick_add_node_thresh();
            behavior_ctx.max_iterations_find_connection = ctx.cc_max_iterations_find_connection();
            behavior_ctx.max_iterations_find_node = ctx.cc_max_iterations_find_node();
            behavior_ctx.reactivate_connection_thresh = ctx.cc_reactivate_connection_thresh();
            behavior_ctx.selected_and_possessed = ctx.cc_selected_and_possessed(creature_circle);
            behavior_ctx.left_key_down = ctx.cc_left_key_down();
            behavior_ctx.right_key_down = ctx.cc_right_key_down();
            behavior_ctx.space_key_down = ctx.cc_space_key_down();
            behavior_ctx.spawn_circle = [&](std::unique_ptr<EatableCircle> c) { ctx.cc_spawn_circle(std::move(c)); };
            creature_circle->set_behavior_context(behavior_ctx);
            CreatureCircle::DivisionContext division_ctx{};
            division_ctx.circle_density = ctx.cc_circle_density();
            division_ctx.init_mutation_rounds = ctx.cc_init_mutation_rounds();
            division_ctx.init_add_node_thresh = ctx.cc_init_add_node_thresh();
            division_ctx.init_add_connection_thresh = ctx.cc_init_add_connection_thresh();
            division_ctx.linear_impulse_magnitude = ctx.cc_linear_impulse_magnitude();
            division_ctx.angular_impulse_magnitude = ctx.cc_angular_impulse_magnitude();
            division_ctx.linear_damping = ctx.cc_linear_damping();
            division_ctx.angular_damping = ctx.cc_angular_damping();
            division_ctx.mutation_rounds = ctx.cc_mutation_rounds();
            division_ctx.mutate_weight_thresh = ctx.cc_mutate_weight_thresh();
            division_ctx.mutate_weight_full_change_thresh = ctx.cc_mutate_weight_full_change_thresh();
            division_ctx.mutate_weight_factor = ctx.cc_mutate_weight_factor();
            division_ctx.add_connection_thresh = ctx.cc_add_connection_thresh();
            division_ctx.max_iterations_find_connection = ctx.cc_max_iterations_find_connection();
            division_ctx.reactivate_connection_thresh = ctx.cc_reactivate_connection_thresh();
            division_ctx.add_node_thresh = ctx.cc_add_node_thresh();
            division_ctx.max_iterations_find_node = ctx.cc_max_iterations_find_node();
            division_ctx.sim_time = ctx.cc_sim_time();
            creature_circle->set_division_context(division_ctx);
            creature_circle->process_eating(ctx.sim_world_id(), ctx, ctx.sim_poison_death_probability(), ctx.sim_poison_death_probability_normal());
...
void GameSimulationController::run_brain_updates(const b2WorldId& worldId, float timeStep) {
    ...
    while (ctx.sim_brain_time_accum() >= brain_period) {
        for (size_t i = 0; i < ctx.sim_circles().size(); ++i) {
            if (ctx.sim_circles()[i] && ctx.sim_circles()[i]->get_kind() == CircleKind::Creature) {
                auto* creature_circle = static_cast<CreatureCircle*>(ctx.sim_circles()[i].get());
                creature_circle->set_minimum_area(ctx.cc_minimum_area());
                creature_circle->set_display_mode(!ctx.cc_show_true_color());
                creature_circle->set_contact_context(ctx.sim_contact_graph(), ctx.sim_circle_registry(), ctx.sim_petri_radius());
                CreatureCircle::BehaviorContext behavior_ctx{};
                behavior_ctx.boost_area = ctx.cc_boost_area();
                behavior_ctx.circle_density = ctx.cc_circle_density();
